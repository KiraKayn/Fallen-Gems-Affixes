...
public class ChainCastAffix extends Affix {
    ...
    public static final Codec<ChainCastAffix> CODEC = RecordCodecBuilder.create(inst -> inst.group(
            affixDef(),
            SpellRegistry.REGISTRY.byNameCodec().optionalFieldOf("trigger_spell").forGetter(a -> a.triggerSpell),
            SpellRegistry.REGISTRY.byNameCodec().fieldOf("spell").forGetter(a -> a.spell),
            Codec.INT.optionalFieldOf("cooldown_between_casts", MIN_DELAY_TICKS).forGetter(a -> a.cooldownBetweenCasts),
            LootRarity.mapCodec(SpellCastAffix.TriggerData.CODEC).fieldOf("values").forGetter(a -> a.values),
            Codec.INT.optionalFieldOf("cooldown", 0).forGetter(a -> a.cooldown),
            LootCategory.SET_CODEC.fieldOf("types").forGetter(a -> a.types),
            SpellCastAffix.TargetType.CODEC.optionalFieldOf("target").forGetter(a -> a.target)
    ...
    public ChainCastAffix(AffixDefinition def, Optional<AbstractSpell> triggerSpell, AbstractSpell spell,
                          int cooldownBetweenCasts, Map<LootRarity, SpellCastAffix.TriggerData> values,
                          int cooldown, Set<LootCategory> types, Optional<SpellCastAffix.TargetType> target) {
        super(def);
    ...
    public void onSpellCast(LivingEntity caster, String castSpellId, LivingEntity spellTarget, LootRarity rarity) {
        if (caster.level().isClientSide()) return;
        if (SpellCastAffix.isCurrentlyTriggering(caster)) return;
        if (isOnCooldown(this.id().toString(), caster)) return;
        ...
        if (this.cooldownBetweenCasts < MIN_DELAY_TICKS) {
            FallenGemsAffixes.LOGGER.error(
                    ...
                    this.id(), this.cooldownBetweenCasts, MIN_DELAY_TICKS, MIN_DELAY_TICKS);
            return;
        }
        ...
        if (cooldownTicks > 0) startCooldown(this.id().toString(), caster, cooldownTicks);
        ...
        try {
            if (caster.level() instanceof ServerLevel sl) {
                ...
                if (lastTargetId != null) {
                    ...
                    if (lastTarget instanceof LivingEntity le && !le.isRemoved()) le.invulnerableTime = 0;
                }
            }
            if (!actualTarget.isRemoved()) actualTarget.invulnerableTime = 0;
            SpellCastUtil.castSpell(caster, this.spell, spellLevel, actualTarget);
        } catch (Exception e) {
            FallenGemsAffixes.LOGGER.warn("ChainCastAffix: spell {} cast failed: {}",
    ...
    private LivingEntity resolveTarget(LivingEntity caster, LivingEntity defaultTarget) {
        if (this.target.isPresent() && this.target.get() == SpellCastAffix.TargetType.SELF) return caster;
    ...
    public MutableComponent getDescription(AffixInstance inst, AttributeTooltipContext ctx) {
        return buildDescription(inst.getRarity(), inst.level());
    }
    ...
    private MutableComponent buildDescription(LootRarity rarity, float affixLevel) {
        ...
        MutableComponent comp = Component.translatable(
                "affix.fallen_gems_affixes.chain_cast", coloredSpellName
        ...
        Component triggerName = this.triggerSpell
                ...
                .orElseGet(() -> Component.translatable("affix.fallen_gems_affixes.chain_cast.any_spell")
                        .withStyle(ChatFormatting.YELLOW));
        comp.append(Component.translatable("affix.fallen_gems_affixes.chain_cast.trigger", triggerName)
                .withStyle(ChatFormatting.YELLOW));
        ...
        if (cooldownTicks != 0) {
            comp.append(Component.literal(" ")
                    .append(Component.translatable("affix.apotheosis.cooldown",
                            StringUtil.formatTickDuration(cooldownTicks, 20f))
                    .withStyle(ChatFormatting.YELLOW)));
        }
        return comp;
    }
