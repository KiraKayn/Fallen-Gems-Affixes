...
public class SpellCastAffix extends Affix {
    ...
    public static void setTriggering(LivingEntity caster, boolean value) {
        if (value) ACTIVE_TRIGGERS.put(caster.getUUID(), caster.level().getGameTime() + 3);
        else ACTIVE_TRIGGERS.remove(caster.getUUID());
    }
    ...
    public static final Codec<SpellCastAffix> CODEC = RecordCodecBuilder.create(inst -> inst.group(
            affixDef(),
            SpellRegistry.REGISTRY.byNameCodec().fieldOf("spell").forGetter(affix -> affix.spell),
            ...
            Codec.INT.optionalFieldOf("cooldown", 0).forGetter(affix -> affix.cooldown),
    ...
    public SpellCastAffix(AffixDefinition def, AbstractSpell spell, TriggerType trigger,
                          Map<LootRarity, TriggerData> values, int cooldown,
                          Set<LootCategory> types, Optional<TargetType> target) {
        super(def);
        this.spell = spell;
    ...
    public void triggerSpell(LivingEntity caster, LivingEntity target, LootRarity rarity, int spellLevelOverride) {
        ...
        if (!hasActiveRecast && isOnCooldown(this.id().toString(), cooldownTicks, caster)) return;
        if (!hasActiveRecast) startCooldown(this.id().toString(), caster, cooldownTicks);
        ...
        DelayedTaskScheduler.schedule(caster.level(), 1, () -> {
            ...
            try {
                if (caster.level() instanceof net.minecraft.server.level.ServerLevel sl) {
                    UUID lastTargetId = net.kayn.fallen_gems_affixes.event.SpellEventHandler.LAST_SPELL_DAMAGE_TARGET.get(caster.getUUID());
    ...
    private LivingEntity determineTarget(LivingEntity caster, LivingEntity defaultTarget) {
        return this.target.map(targetType -> targetType == TargetType.SELF ? caster : defaultTarget)
    ...
    public void doPostAttack(AffixInstance inst, LivingEntity user, @Nullable Entity target) {
        if (this.trigger == TriggerType.MELEE_HIT && target instanceof LivingEntity livingTarget) {
            ...
            triggerSpell(user, actualTarget, inst.getRarity(), 0);
        }
    }
    ...
    public void doPostHurt(AffixInstance inst, LivingEntity user, DamageSource source) {
        if (this.trigger == TriggerType.HURT) {
            ...
            LivingEntity defaultTarget = this.target
                    .map(t -> t == TargetType.TARGET && attacker instanceof LivingEntity le ? le : user)
                    .orElse(user);
    ...
    public record TriggerData(LevelRange level, int cooldown) {
        public static final Codec<TriggerData> CODEC = RecordCodecBuilder.create(inst -> inst.group(
                ...
                Codec.INT.optionalFieldOf("cooldown", -1).forGetter(TriggerData::cooldown)
