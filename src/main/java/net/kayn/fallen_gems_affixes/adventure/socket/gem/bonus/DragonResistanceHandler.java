...
public class DragonResistanceHandler {
    ...
    public static void onLivingHurt(LivingHurtEvent event) {
        ...
        if (!BossUtil.isBoss(attacker, DRAGON_TAG)) return;
        ...
        for (EquipmentSlot slot : EquipmentSlot.values()) {
            ...
            SocketHelper.getGems(armor).forEach(gemInstance -> {
                ...
                gemInstance.gem().get().getBonus(category, rarity).ifPresent(bonus -> {
                    if (bonus instanceof DragonResistanceBonus resistance && resistance.supports(rarity)) {
