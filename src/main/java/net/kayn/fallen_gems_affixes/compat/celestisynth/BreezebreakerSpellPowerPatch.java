...
public class BreezebreakerSpellPowerPatch {
    ...
    private static Holder<Attribute> evocationSpellPower = null;
    private static boolean lookedUp = false;

    private static Holder<Attribute> getEvocationSpellPower() {
        if (!lookedUp) {
            lookedUp = true;
            if (HAS_IRONS) evocationSpellPower = BuiltInRegistries.ATTRIBUTE.getHolder(EVOCATION_SPELL_POWER_ID).orElse(null);
        }
        return evocationSpellPower;
    }
    ...
    public static void onLivingHurt(LivingIncomingDamageEvent event) {
        ...
        if (!held.is(BuiltInRegistries.ITEM.get(BREEZEBREAKER_ID))) return;

        Holder<Attribute> attr = getEvocationSpellPower();
        if (attr == null) return;
