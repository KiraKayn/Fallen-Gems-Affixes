...
public class KeresSpellPowerPatch {
    ...
    private static Holder<Attribute> bloodSpellPower = null;
    private static boolean lookedUp = false;

    private static Holder<Attribute> getBloodSpellPower() {
        if (!lookedUp) {
            lookedUp = true;
            if (HAS_IRONS) bloodSpellPower = BuiltInRegistries.ATTRIBUTE.getHolder(BLOOD_SPELL_POWER_ID).orElse(null);
        }
        return bloodSpellPower;
    }
    ...
    public static void onLivingHurt(LivingIncomingDamageEvent event) {
        ...
        if (!held.is(BuiltInRegistries.ITEM.get(KERES_ID))) return;

        Holder<Attribute> attr = getBloodSpellPower();
        if (attr == null) return;
