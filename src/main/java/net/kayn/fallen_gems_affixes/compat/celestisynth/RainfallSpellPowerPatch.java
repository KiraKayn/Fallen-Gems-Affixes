...
public class RainfallSpellPowerPatch {
    ...
    private static Holder<Attribute> lightningSpellPower = null;
    private static boolean lookedUp = false;

    private static Holder<Attribute> getLightningSpellPower() {
        if (!lookedUp) {
            lookedUp = true;
            if (HAS_IRONS) lightningSpellPower = BuiltInRegistries.ATTRIBUTE.getHolder(LIGHTNING_SPELL_POWER_ID).orElse(null);
        }
        return lightningSpellPower;
    }
    ...
    public static void onLivingHurt(LivingIncomingDamageEvent event) {
        ...
        if (!held.is(BuiltInRegistries.ITEM.get(RAINFALL_SERENITY_ID))) return;

        Holder<Attribute> attr = getLightningSpellPower();
        if (attr == null) return;
