...
public class SolarisSpellPowerPatch {
    ...
    private static Holder<Attribute> fireSpellPower = null;
    private static boolean lookedUp = false;

    private static Holder<Attribute> getFireSpellPower() {
        if (!lookedUp) {
            lookedUp = true;
            if (HAS_IRONS) fireSpellPower = BuiltInRegistries.ATTRIBUTE.getHolder(FIRE_SPELL_POWER_ID).orElse(null);
        }
    ...
    public static void onLivingHurt(LivingIncomingDamageEvent event) {
        ...
        if (!held.is(BuiltInRegistries.ITEM.get(SOLARIS_ID))) return;

        Holder<Attribute> attr = getFireSpellPower();
        if (attr == null) return;
