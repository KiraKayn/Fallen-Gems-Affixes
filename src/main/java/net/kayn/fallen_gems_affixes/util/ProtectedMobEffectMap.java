...
public class ProtectedMobEffectMap<E extends LivingEntity> extends HashMap<Holder<MobEffect>, MobEffectInstance>{
    private final E owner;
    private final Map<Holder<MobEffect>, EffectInstanceBucket> fallback = new HashMap<>();
    private final Map<EquipmentSlotWrapper, Set<Holder<MobEffect>>> cachedEffectsBySlot = new HashMap<>();
    private final Set<Holder<MobEffect>> currentPermanentEffects = new HashSet<>();
    ...
    @Override
    public MobEffectInstance put(Holder<MobEffect> effect, MobEffectInstance effectInst) {
    ...
    public MobEffectInstance remove(Object key) {
        ...
        if (operator == EffectOperator.EXTERNAL) {
             if(this.currentPermanentEffects.contains((Holder<MobEffect>) key)) {
                 return null;
             }
        }
        else {
            ...
            if (effectInst != null) {
                tryRemovePermanentEffect(currentSlot.get(), (Holder<MobEffect>) key, effectInst.getAmplifier(), false);
            }
            return effectInst;
        }
        return super.remove(key);
    ...
    public boolean remove(Object key, Object value) {
        ...
        if (operator == EffectOperator.EXTERNAL) {
            ...
            if (value != null) {
                tryRemovePermanentEffect(currentSlot.get(), (Holder<MobEffect>) key,((MobEffectInstance) value).getAmplifier(), false);
            }
            return true;
        }
        return super.remove(key, value);
    ...
    public boolean containsPermanent(Holder<MobEffect> effect) {
        return currentPermanentEffects.contains(effect);
    }

    public Set<Holder<MobEffect>> getEffectsFromCache(EquipmentSlotWrapper slot) {
    ...
    public void addPermanentEffect(EquipmentSlotWrapper slot, Holder<MobEffect> effect, int amplifier, boolean isAltCondition) {
    ...
    private void updateCachedMobEffectsForSlot(EquipmentSlotWrapper slot, Holder<MobEffect> effect, PermanentEffectHandler.Operation operation) {
        Set<Holder<MobEffect>> effects = cachedEffectsBySlot.get(slot);
        if(effects == null) {
            effects = new HashSet<>();
    ...
    public void tryRemovePermanentEffect(EquipmentSlotWrapper slot, Holder<MobEffect> effect, int amplifier, boolean altCondition) {
        if (effect == null) return;
    ...
    private void tryUpdateCachedMobEffectsForSlot(EquipmentSlotWrapper slot, Holder<MobEffect> effect, PermanentEffectHandler.Operation operation, boolean isAltCondition) {
        if (owner.level().isClientSide || isAltCondition) {
            updateCachedMobEffectsForSlot(slot, effect, operation);
        }
    }

    public MobEffectInstance getLastPotentialEffectInst(Holder<MobEffect> effect) {
